        ...
        <div class="bg-white p-6 mb-6">
            ...
            <form action="{{ route('kriteria.store') }}" method="POST" class="space-y-4">
                ...
                <select id="jenis_beasiswa_id" name="jenis_beasiswa_id" class="w-full p-3 border rounded-lg shadow-sm" required>
                    ...
                    <option value="{{ $beasiswa->id }}" {{ (old('jenis_beasiswa_id') == $beasiswa->id || (isset($kriteria) && $kriteria->jenis_beasiswa_id == $beasiswa->id)) ? 'selected' : '' }}>{{ $beasiswa->nama }}</option>
                    @endforeach
                </select>
                    ...
                    <tr class="bg-white" data-beasiswa="{{ strtolower($kriteria->jenisBeasiswa->nama ?? '') }}">
                        ...
                        <td class="border px-6 py-2 text-center">
                            <div class="flex justify-center items-center space-x-3">
                                <a href="{{ route('kriteria.edit', $kriteria->id) }}" class="text-yellow-500 hover:text-yellow-700">
