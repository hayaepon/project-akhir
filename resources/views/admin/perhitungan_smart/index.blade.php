...
<div class="container mx-auto px-4 py-6 min-h-screen">
    <div class="bg-white p-6 rounded shadow">
        ...
        <form action="{{ route('admin.perhitungan_smart.store') }}" method="POST" class="space-y-6">
            ...
            <!-- Container Kriteria dinamis -->
            <div id="kriteria-container" class="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
                {{-- Kriteria dan subkriteria akan muncul di sini via JavaScript --}}
            </div>

           <div class="flex space-x-4 justify-start">
                    <button type="submit" class="bg-green-600 text-white py-2 px-6 rounded-lg shadow-md">Simpan</button>
                    <a href="{{ route('admin.perhitungan_smart.index') }}" class="bg-yellow-400 text-white py-2 px-8 rounded-lg shadow-md">Batal</a>
                </div>
    </div>

    <!-- Tabel Hasil Perhitungan (Placeholder, sesuaikan dengan data di controller jika ada) -->
        ...
        <table class="min-w-full border border-gray-300 border-collapse">
            <thead class="bg-blue-700 text-white">
                <tr>
                    ...
                    {{-- Header kriteria dinamis jika mau, contoh statis dulu --}}
            ...
            <tbody>
    @forelse ($perhitunganSmarts as $perhitungan)
        <tr>
            ...
            {{-- Ambil nilai kriteria dari array --}}
            @php
                $nilai = $perhitungan->kriteria;
            @endphp
            <td class="border px-4 py-2">{{ $nilai[0] ?? '-' }}</td>
            <td class="border px-4 py-2">{{ $nilai[1] ?? '-' }}</td>
            <td class="border px-4 py-2">{{ $nilai[2] ?? '-' }}</td>
    ...
    @empty
        <tr>
            <td colspan="7" class="text-center py-4 text-gray-600">Belum ada data hasil perhitungan.</td>
        </tr>
    @endforelse
</tbody>
...
{{-- Script untuk load kriteria berdasarkan beasiswa --}}
<script>
document.addEventListener('DOMContentLoaded', () => {
            ...
            .then(data => {
                ...
                if (data.length === 0) {
                    ...
                    data.forEach(kriteria => {
                        const kriteriaInput = createKriteriaInput(kriteria);
                        kriteriaContainer.appendChild(kriteriaInput);
                    });
                }
            })
            .catch(err => {
    ...
    // Jika sebelumnya ada old input, maka muat ulang kriteria
